...
- `src/content/site.ts` — single source of truth for all copy, links, work items, and case-study content. `WorkItem` (`real: true` items need `slug`, `description`, `live`, `github`, `shots: Shot[]`) and `CaseStudy` (`problem`/`architecture`/`decisions`/`outcome`) types live here too. `Shot` carries `width`/`height` (the file's real pixel dimensions) alongside `src`/`alt` — images render at natural aspect ratio (`next/image` with explicit `width`/`height`, `className="h-auto w-full"`, no `fill`/`object-contain`/box background), so a wrong width/height pair here will visibly stretch that image.
- `src/app/page.tsx` — single-page layout: header, hero (name + tagline + headshot photo), specialization/about, work grid, contact.
- `src/app/work/[slug]/page.tsx` — case-study detail route (real Next.js routing, not a client-side view-state toggle like the original Claude Design mockup used — URL-addressable pages are the better fit for this framework). Renders `caseStudies[slug]` from `site.ts`; 404s via `notFound()` for unknown slugs. `params` is a Promise in this Next.js version — must `await` it. Also looks up the matching `site.work` item by slug and renders its full `shots` array as a 2-column gallery (natural aspect ratio per image, same as the card) below the meta bar.
...
- `src/components/ProjectCard.tsx` — client component. Real work items show only `shots[0]` as a static cover image (the rest of the gallery lives on the case-study page, not cycling on the card anymore). Images use `next/image` with real `width`/`height` and `className="h-auto w-full"` — never `fill`/`object-contain`/a fixed-aspect box, which left grey letterboxing for shots that don't match a 16:10 ratio. Non-real (`real: false`) items render as dashed-border "open project slot" placeholders.
...
- **Productivy** (`productivy`) — 4 real screenshots (landing-page cover hidden from gallery, then projects/project-board/dashboard in the gallery — dashboard is `wide` since it's taller), real public GitHub link. Case study sourced from `../Project Summaries/Productivy/Productivy.md`.
